...
export const blogmeta = {
  ...
  date: '2021-05-11 08:53:21', // Ctrl+Shift+I w/ Insert Date String plugin
  authors: [pauldevito],
...
- Building an Event Driven .NET Application: Setting Up MassTransit and RabbitMQ (TBD)
...
An Event Driven Architecture (EDA) is used in distributed applications to pass messages around using events to send a notification some kind. This notification will signify that some change significant state change has occurred so that other applications can react to that state change in whatever way is required by the business. For example, say a customer changes their address in an application, that application might send out a `CustomerAddressUpdated` event and any other application that needs to react to that address change can do so. 
...
<img class="mx-auto" src="https://www.cloudamqp.com/img/blog/workflow-rabbitmq.png" alt="basic rabbitmq workflow" />
...
<img class="mx-auto" src="https://www.cloudamqp.com/img/blog/exchanges-bidings-routing-keys.png" alt="expanded broker diagram" />
...
<img class="mx-auto max-w-sm sm:max-w-md md:max-w-lg lg:max-w-2xl" src="https://www.cloudamqp.com/img/blog/exchanges-topic-fanout-direct.png" alt="basic exchange diagram" />
