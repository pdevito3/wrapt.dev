...
export const blogmeta = {
  ...
  description: `Let's expand on the prior Duende BFF post and see how to set up a standalone Duende BFFthat works with any SPA!`,
...
- The Authentication service registartion is fairly normal
  ...
  - The `AddOpenIdConnect` configuration is setting up our auth server connection information. In this case, I have keycloak running in docker.
    ...
    - The response type should be `code` when going througha user interaction flow like this
...
Next I'm going to add tailwind [according to the vite setup](https://tailwindcss.com/docs/guides/vite). This is totally optional, but makes styling much nicer
...
1. A loading screen when the api call is occuring
2. A login butt if you're not logged in
3. Your name and a logout button if you are logged in
4. A list of recipes from our api (to deonstrate that we can hit a secure endpoint on another api)
...
The `/api` proxy is so we can call an api that requires proper authenication to access it. In practice, you'll probably want to change this to something more api specific since you'll potentially be talking to multiple apis.
