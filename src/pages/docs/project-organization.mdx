...
Note that Wrapt APIs do *not* use Sieve's pagination capabilities. For mroe information on how Wrapt handles pagination, see the [pagination](#pagination) section.
...
To add a filter to your API calls, just add a `Filters` query string and the designated filter values youd like to use. Below is a few examples, but you can find a full list of 
...
Sorting can also be added using the `SortOrder` query string and passing a comma separated list in the order you'd like to sort by. You can use a preceding `-` to sort descendingly.
```http
...
#### Pagination Requests
When making a request to your GET collection endpoint, you can pass a `PageNumber` and `PageSize` query string, or exlcude them to use the default values. 
