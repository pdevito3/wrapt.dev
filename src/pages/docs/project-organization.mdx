...
These entities will always inherit from a `BaseEntity` with a Guid `Id` property for consistency across your domain.
...
but you can add any rules that fit your business needs in `Domain.ENTITYPLURAL.Validation`. Validation rules can be assigned to the `ENTITYForManipulationDtoValidator` class 
...
#### Adding Tests for MassTransit
Craftsman will scaffold out a myriad of tests for you, but will not do this for consumers or producers at this time. If you want to add these, you'll need to add them manually for the time being. 

For consumers, Craftsman will register your consumers in the test harness of integration testing project whenever you add them in, so you should generally be able to just add a test without much hassle.
...
public async Task bus_consumer_test()
{
    ...
    // the particular consumer i'm testing consumed the message
    var consumerHarness = _provider.GetRequiredService<ISendReportRequest<SendEmailReport>>();
    Assert.That(await consumerHarness.Consumed.Any<ISendReportRequest>());
}
```