...
### Encapsulated Domain
Generally, we want to push our behaviors down into our domain model or (a domain layer if the level of complexity calls for it). This means that our handler gets thinner and thinner and is really just acting as a data access logic (no need for an extra repository layer). 
Then we can get delegate all the meat of the work in the domain logic. This means we have a fully encapsulated domain.
...
- **Unit tests** are meant to confirm that individual operations are working as expected 
(e.g. PagedList calculations). 
...
and run each of your feature tests in an actual representation of your database. 
