...
If you have Machine to Machine communications, then you should be using the `ClientCredentials` flow. What does that mean though?

Basically you have a client, an authorization server (Duende, Okta, etc. to make sre you are who you say you are), and you have resources/apis. 
...
It is worth noting that this example uses a client secret which, while acceptable, can be hardened using asymmetric keys. For further discussion see the [M2M portion of Dominick Baier's NDC 2020 talk](https://youtu.be/jeRALmfyoqg?t=1113).
...
This is a more involved process than the `ClientCredentials` flow. Using this type of `code` flow, we start by sending a request to the authorization server.

To get an authorization token that proves that we are who we say we are, we go through a specific flow to get that token. 
