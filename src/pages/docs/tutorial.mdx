...
To get things started,  we need to start out by building a bare bones `yaml` or `json` file to lay out what we want our project to look like. This file is going to describe what our particular domain looks like 
...
Let's start out with the `Recipes` bounded context and name it with a property called `SolutionName`. 
...
- SolutionName: Shopping
  ...
  DbContext:
   ...
   DatabaseName: ShoppingDbContext
   Provider: Postgres
  Entities:
  - Name: ShoppingList
    Properties:
